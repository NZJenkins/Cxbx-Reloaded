            ...
            switch (State) {
                ...
                case XTL::X_D3DTSS_ADDRESSU: case XTL::X_D3DTSS_ADDRESSV: case XTL::X_D3DTSS_ADDRESSW:
                    switch (Value) {
                        ...
                        case XTL::X_D3DTADDRESS_WRAP:        // = 1 = D3DTADDRESS_WRAP = 1,
                        case XTL::X_D3DTADDRESS_MIRROR:      // = 2 = D3DTADDRESS_MIRROR = 2,
                        case XTL::X_D3DTADDRESS_CLAMP:       // = 3 = D3DTADDRESS_CLAMP = 3,
                        case XTL::X_D3DTADDRESS_BORDER:      // = 4 = D3DTADDRESS_BORDER = 4,
                            // These match host Direct3D 9 values, so no update necessary
                            break;
                        case XTL::X_D3DTADDRESS_CLAMPTOEDGE: // = 5
                ...
                case XTL::X_D3DTSS_MAGFILTER: case XTL::X_D3DTSS_MINFILTER: case XTL::X_D3DTSS_MIPFILTER:
                    switch (Value) {
                        case XTL::X_D3DTEXF_NONE:        // = 0 = D3DTEXF_NONE = 0,        // filtering disabled (valid for mip filter only)
                        case XTL::X_D3DTEXF_POINT:       // = 1 = D3DTEXF_POINT = 1,       // nearest
                        case XTL::X_D3DTEXF_LINEAR:      // = 2 = D3DTEXF_LINEAR = 2,      // linear interpolation
                        case XTL::X_D3DTEXF_ANISOTROPIC: // = 3 = D3DTEXF_ANISOTROPIC = 3, // anisotropic
                            // These match host Direct3D 9 values, so no update necessary
                            break;
                        case XTL::X_D3DTEXF_QUINCUNX:    // = 4; // quincunx kernel (Xbox extension), also known as "flat cubic"
                            LOG_TEST_CASE("X_D3DTEXF_QUINCUNX unsupported, falling back to D3DTEXF_ANISOTROPIC");
                            Value = D3DTEXF_ANISOTROPIC;
                            break;
                        case XTL::X_D3DTEXF_GAUSSIANCUBIC: // = 5 // Xbox extension, different cubic kernel
