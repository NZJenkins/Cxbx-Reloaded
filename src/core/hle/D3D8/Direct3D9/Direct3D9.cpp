...
void Direct3D_CreateDevice_End()
{
    ...
    if (g_pXbox_BackBufferSurface == xbnullptr) {
        ...
        CxbxImpl_SetRenderTarget(g_pXbox_BackBufferSurface, xbnullptr);
    }
    ...
    if (g_pXbox_DefaultDepthStencilSurface == xbnullptr) {
        ...
        if (g_pXbox_DefaultDepthStencilSurface == xbnullptr) {
            ...
            CxbxImpl_SetRenderTarget(xbnullptr, g_pXbox_DefaultDepthStencilSurface);
        }
    }
}
...
void CxbxUpdateHostTextureScaling()
{
	...
	std::array<std::array<float, 4>, XTL::X_D3DTS_STAGECOUNT> texcoordScales = { 0 };
	for (int stage = 0; stage < XTL::X_D3DTS_STAGECOUNT; stage++) {
		auto pXboxBaseTexture = g_pXbox_SetTexture[stage];
		...
		if (g_Xbox_VertexShader_IsFixedFunction) {
			...
			auto texCoordIndexState = XboxTextureStates.Get(stage, XTL::X_D3DTSS_TEXCOORDINDEX);
