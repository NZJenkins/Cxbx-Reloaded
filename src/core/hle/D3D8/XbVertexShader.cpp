...
DeferredVertexShader::DeferredVertexShader(std::future<ID3DBlob*> shaderFuture) {
	compileResult = std::move(shaderFuture);
}

DeferredVertexShader::DeferredVertexShader() {
	isReady = true;
	pHostVertexShader = nullptr;
}

IDirect3DVertexShader* DeferredVertexShader::get(IDirect3DDevice* device) {
	if (isReady)
	...
	try {
		...
		pCompiledShader = compileResult.get();
