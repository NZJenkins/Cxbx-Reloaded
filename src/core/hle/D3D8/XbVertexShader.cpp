...
)
{
	...
	*pXboxFunctionSize = 0;
	...
	auto headerOffset = headerless ? 0 : sizeof(XTL::X_VSH_SHADER_HEADER) / sizeof(DWORD);

	// Decode the vertex shader program tokens into an intermediate representation
	*ppXboxShaderInstructions = &pXboxFunction[headerOffset];
	uint32_t* pCurToken = (uint32_t*)*ppXboxShaderInstructions;
