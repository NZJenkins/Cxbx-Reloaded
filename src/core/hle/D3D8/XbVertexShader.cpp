...
// Variables set by [D3DDevice|CxbxImpl]_SetVertexShader() and [D3DDevice|CxbxImpl]_SelectVertexShader() :
                        bool g_Xbox_VertexShader_IsFixedFunction = true; // Corresponds to NV2A_ENGINE_FIXED / NV097_SET_TRANSFORM_EXECUTION_MODE_MODE_FIXED
     ...
     XTL::X_D3DVertexShader *g_Xbox_VertexShader_Ptr = nullptr;
#endif
...
// Variable set by [D3DDevice|CxbxImpl]_LoadVertexShader() / [D3DDevice|CxbxImpl]_LoadVertexShaderProgram() (both through CxbxCopyVertexShaderFunctionSlots):
                  XTL::DWORD g_Xbox_VertexShader_FunctionSlots[(X_VSH_MAX_INSTRUCTION_COUNT + 1) * X_VSH_INSTRUCTION_SIZE] = { 0 }; // One extra for FLD_FINAL terminator
...
static XTL::X_D3DVertexShader* XboxVertexShaderFromFVF(DWORD xboxFvf)
{
	using namespace XTL;
...
static XTL::X_D3DVertexShader* CxbxGetXboxVertexShaderForHandle(DWORD Handle)
{
...
static bool UseXboxD3DVertexShaderTypeForVersion3948(XTL::X_D3DVertexShader* pXboxVertexShader)
{
	// Don't check XDK version for our internal FVF vertex shader
	// because g_Xbox_VertexShader_ForFVF is an internal variable
	// that's compiled in as a XTL::X_D3DVertexShader
...
static XTL::X_VERTEXATTRIBUTEFORMAT* CxbxGetVertexShaderAttributes(XTL::X_D3DVertexShader* pXboxVertexShader)
{
	if (UseXboxD3DVertexShaderTypeForVersion3948(pXboxVertexShader)) {
		auto pXboxVertexShader3948 = (XTL::X_D3DVertexShader3948*)pXboxVertexShader;
...
static DWORD* CxbxGetVertexShaderTokens(XTL::X_D3DVertexShader* pXboxVertexShader, DWORD* pNrTokens)
{
	if (UseXboxD3DVertexShaderTypeForVersion3948(pXboxVertexShader)) {
		auto pXboxVertexShader3948 = (XTL::X_D3DVertexShader3948*)pXboxVertexShader;
...
XTL::X_STREAMINPUT& GetXboxVertexStreamInput(unsigned XboxStreamNumber)
{
...
private:
	...
	D3DDECLUSAGE Xb2PCRegisterType(DWORD VertexRegister, BYTE &UsageIndex)
	{
		...
		switch (VertexRegister) {
			case XTL::X_D3DVSDE_POSITION    /*= 0*/:                 return D3DDECLUSAGE_POSITION;
...
public:
	D3DVERTEXELEMENT* Convert(XTL::X_VERTEXATTRIBUTEFORMAT* pXboxDeclaration, bool bIsFixedFunction, CxbxVertexDeclaration* pCxbxVertexDeclaration)
	{
		...
		for (size_t i = 0; i < orderedRegisterIndices.size(); i++) {
			...
			if (slot.Format > XTL::X_D3DVSDT_NONE) {
...
VertexDeclarationKey GetXboxVertexAttributesKey(XTL::X_VERTEXATTRIBUTEFORMAT* pXboxVertexAttributeFormat)
{
	auto attributeHash = ComputeHash((void*)pXboxVertexAttributeFormat, sizeof(XTL::X_VERTEXATTRIBUTEFORMAT));
...
CxbxVertexDeclaration* CxbxGetVertexDeclaration()
{
	...
	XTL::X_VERTEXATTRIBUTEFORMAT *pXboxVertexAttributeFormat = GetXboxVertexAttributeFormat();
...
void CxbxImpl_SetVertexShader(DWORD Handle)
{
	...
	XTL::X_D3DVertexShader* pXboxVertexShader = CxbxGetXboxVertexShaderForHandle(Handle);
	g_Xbox_VertexShader_IsPassthrough = false;
